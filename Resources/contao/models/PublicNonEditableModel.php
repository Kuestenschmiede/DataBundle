    ...
    public static function find() {
        ...
        if (PublicNonEditableModule::$dataMode === '1' && !empty(PublicNonEditableModule::$type)) {
            ...
            foreach (PublicNonEditableModule::$type as $type) {
                $stmtElements = $db->prepare("SELECT tl_c4g_data_element.* FROM tl_c4g_data_element JOIN tl_c4g_data_type ON tl_c4g_data_element.type = tl_c4g_data_type.id WHERE tl_c4g_data_element.type = ? AND (tl_c4g_data_type.allowPublishing != 1 OR tl_c4g_data_element.published = 1) ORDER BY name ASC");
            ...
            if (PublicNonEditableModule::$dataMode === '2' && !empty(PublicNonEditableModule::$directory)) {
                ...
                $stmtElements = $db->prepare("SELECT tl_c4g_data_element.* FROM tl_c4g_data_element JOIN tl_c4g_data_type ON tl_c4g_data_element.type = tl_c4g_data_type.id WHERE (tl_c4g_data_type.allowPublishing != 1 OR tl_c4g_data_element.published = 1) ORDER BY name ASC");
